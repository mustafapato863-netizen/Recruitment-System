...
test('accepts a paused mail worker without SMTP settings', () => assert.equal(run({
  MAIL_DELIVERY_ENABLED: 'false', MAIL_TRANSPORT: 'console', SMTP_HOST: '', SMTP_USER: '', SMTP_PASS: '', MAIL_FROM: '',
}).status, 0));
for (const [name, override] of Object.entries({
  ...
  invalidMailDeliveryFlag: { MAIL_DELIVERY_ENABLED: 'paused' },
