...
Approved product decision (2026-08-18): RecruitFlow supports explicit Light and Dark modes. Light is the first-visit default; both modes use the canonical semantic tokens and require equivalent accessibility and responsive evidence. This decision supersedes every historical light-only instruction in this repository.
...
- Keep Light and Dark on the shared RecruitFlow semantic tokens; do not add a third theme.
...
- Never add a color mode beyond the approved Light and Dark contract without product approval.
