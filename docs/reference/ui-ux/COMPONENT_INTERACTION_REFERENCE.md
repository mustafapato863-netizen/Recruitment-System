...
## 5. Atmospheric canvas contract

The first version is light mode, with depth coming from a restrained canvas rather than a dark theme or full glassmorphism.

- Use one low-contrast blue/cyan/violet ambient layer behind the shell. The drift target is `20–30s`, and it must stop under `prefers-reduced-motion: reduce`.
- Keep the working surfaces opaque or near-opaque. Candidate PII, salary, approvals, tables, and forms must never sit on an animated background.
- Use `--gradient-action` only for primary actions, `--gradient-selection` for selected navigation/controls, and `--gradient-neon` for one decision-critical focus card per view.
- Create hierarchy in this order: canvas → surface → section/card → focus state → action. Do not make every card neon, elevated, or animated.
- Page entry may reveal the shell once, but the page must remain stable while the user reads or completes work.

## 6. Page archetype contract

Every page must declare one archetype before implementation. Do not combine archetypes unless the handoff explicitly explains the transition.

| Archetype | Primary job | Required recipe | Handoff question |
|---|---|---|---|
| Command Center | Orient and prioritize | Hero/canvas → compact KPIs → one focus card → attention queue | What decision is due next? |
| List / Inbox | Filter and act on records | Toolbar → saved view → table → selection/bulk bar → pagination | What can be safely done to this row or selection? |
| Profile / Detail | Inspect evidence and decide | Identity header → 8/4 content split → activity → decision rail | What evidence supports the next action? |
| Wizard | Create or configure safely | Stepper → 8-column form → 4-column readiness rail | What blocks completion of this step? |
| Pipeline | Move work through stages | Five visible columns → WIP/SLA → wrapped extras → drop feedback | Which stage is constrained and why? |
| Insights / Admin | Understand or configure | Filter-first header → smallest useful chart/table → drilldown/export | What question does this view answer? |

## 7. Motion timing contract

| Interaction | Timing | Rule |
|---|---:|---|
| Page entry | `180–240ms` | Reveal the shell once; do not animate content forever. |
| Hover/focus | `150–220ms` | Animate color, border, shadow, or a 1–2px lift; never flicker. |
| Drawer/popover | `160–220ms` | Enter from the triggering control and preserve focus. |
| Ambient canvas | `20–30s` | Low contrast, behind content, paused for reduced motion. |
| Loading | `~1.5s` shimmer or spinner | Preserve component dimensions and expose an accessible status. |

No motion is allowed to change layout width, move a text label unexpectedly, or soften the meaning of a destructive decision.
