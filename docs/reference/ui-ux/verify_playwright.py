...
def main() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        page.locator("#gallery-search").fill("candidate")
        assert page.locator(".gallery-card:not(.is-hidden)").count() > 0, "catalog search returned no matching screens"
        page.locator("#gallery-search").fill("")
        page.locator(".gallery-filter[data-filter='Hiring & Interviews']").click()
        assert page.locator(".gallery-card:not(.is-hidden)").count() > 0, "catalog chapter filter returned no screens"
        page.locator(".gallery-filter[data-filter='all']").click()
        assert page.locator(".gallery-card:not(.is-hidden)").count() == 56, "catalog reset did not restore all screens"
        ...
        assert page.locator(".reference-command-palette").count() == 1, "command search did not open"
        page.locator(".reference-command-input").fill("candidate")
        assert page.locator(".reference-command-result").count() > 0, "command search returned no result"
        page.keyboard.press("Escape")
        assert page.locator(".reference-command-palette").count() == 0, "command search did not close"
        page.locator(".sidebar-toggle").click()
        assert page.locator("body.nav-collapsed").count() == 1, "sidebar collapse state missing"
        page.locator(".sidebar-toggle").click()
        assert page.locator("body.nav-collapsed").count() == 0, "sidebar expand state missing"
        density = page.locator(".density").nth(1)
        density.click()
        assert density.evaluate("el => el.classList.contains('active')"), "density selector did not update"
        toggle = page.locator(".field-toggle[role='switch']")
        before_toggle = toggle.get_attribute("aria-checked")
        toggle.click()
        assert toggle.get_attribute("aria-checked") != before_toggle, "switch state did not update"
        toggle.click()
        page.locator("table.table-demo tbody .table-select").nth(1).check()
        assert page.locator("#table-selection-summary").text_content().strip() == "2 selected", "table selection summary did not update"
        page.locator("table.table-demo tbody .table-select").nth(1).uncheck()
