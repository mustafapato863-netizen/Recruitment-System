...
function buildGlowVars(glowColor: string, intensity: number) {
  ...
  for (let i = 0; i < opacities.length; i++) {
    vars[`--glow-color${keys[i]}`] = `hsl(${base} / ${Math.min(opacities[i] * intensity, 100)}%)`; // design-token-exception
  }
  return vars;
}
...
export function BorderGlow({
  ...
  const resolvedGlowColor = glowColor ?? (isDark ? '199 89 60' : '196 100 44');
  const resolvedColors = colors ?? (isDark ? ['#38bdf8', '#34d399', '#00a3e0'] : ['#0084ce', '#00a3e0', '#00a859']); /* design-token-exception */
  const resolvedBg = backgroundColor ?? 'var(--color-surface)';
  const resolvedFillOpacity = fillOpacity ?? (isDark ? 0.45 : 0.2);
