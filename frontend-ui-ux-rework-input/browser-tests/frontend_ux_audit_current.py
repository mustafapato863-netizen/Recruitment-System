...
def run() -> int:
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            pipeline = page.get_by_role("button", name=re.compile(r"Open Applicants Pipeline", re.I)).first
            if pipeline.count() and pipeline.is_visible():
                pipeline.click()
            else:
                fallback = page.get_by_role("button", name=re.compile(r"Pipeline", re.I)).first
