    ...
    with sync_playwright() as playwright:
        ...
        evidence = page.evaluate(
            ...
            () => {
              ...
              const metric = document.querySelector('.text-2xl.text-gray-900');
              const card = metric?.closest('.bg-white');
              const heading = document.querySelector('h1');
              const nextAction = [...document.querySelectorAll('button')].find((button) => button.textContent?.includes('POST NEW JOB'));
