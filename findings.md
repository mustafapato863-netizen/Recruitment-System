...
## 9. Enhancements Sweep E2 Findings
- **Offer Page Data Truth & Mock Removal**: `OffersPage` formerly fell back to `DEFAULT_OFFERS` (8 mock rows) whenever `apiOffers` was empty. `DEFAULT_OFFERS` was deleted. Rows are now mapped directly from `apiOffers` with strict null-safe fallbacks: `candidateName ?? 'Unknown candidate'`, `positionTitle ?? 'No position'`, `createdByName || 'Unassigned'`, real computed package sums (`monthlyPackage` or components sum, else `—`), real expiry diffs, and real next action states. When the offers list or filter result is empty, `<PageState kind="empty">` is displayed. Filter/creation modal defaults (`defaultValue="Ali Hassan"` and `defaultValue={28000}`) were replaced with neutral placeholders.
- **Offer Detail Contract Binding & Letter Preview**: `OfferDetailPage` formerly contained hardcoded mock literals for candidate 'Mona Saleh', position 'Registered Nurse – ICU', fake contact information, letter template with hardcoded SGH text, notes seed, static dates ('15 Sep 2026', '1 Sep 2026'), and mock timeline approvers ('Basem Khaled', 'Faisal Almutairi'). These have been replaced with real data from `offer` and `offer.currentVersion` (`candidateName ?? 'Unknown candidate'`, `positionTitle ?? 'No position'`, real components breakdown table, real approval workflow timeline, dynamic letter text generator, and dynamic download filename slug). Contact links (`mailto:` / `tel:`) only render when contact info exists on the application/candidate.
...
## 11. Enhancements Sweep E4b Findings
- **ApplicationsPage Kanban Drag-and-Drop Optimistic Locking & 409 Conflict Handling**: Previously, kanban drops dispatched `patchApi('/applications/:id/stage', { stage, expectedStage, expectedVersion: 1 })` with a hardcoded `expectedVersion: 1` and swallowed all errors (`.catch(() => {})`). In E4b, `KanbanCard` preserves `version: a.version ?? 1` and `stage: a.stage`. On drop, `handleDrop` checks `card.version` (falling back to `GET /applications/:id` if not a number), applies an optimistic update with a previous state snapshot, and sends the P0.1 compliant `UpdateApplicationStageInput` payload (`{ stage, expectedStage, expectedVersion, reason }`). On 409 Conflict (`isConflict`), it notifies the user via a warning toast (`'This application was updated by someone else. Reloading pipeline...'`) and calls `loadApplications()` to resync with server truth. On any other error, it reverts `boardColumns` back to `previousColumns` and surfaces an error toast.
- **ApplicationsPage Mock Literals and Hardcoded Person Data Removal**: `DEFAULT_COLUMNS` cards and `DEFAULT_LIST_ROWS` were completely removed. All mock records ('Sara Mohamed', 'Kareem Tarek', 'Heba Adel', 'Mona Saleh', 'Omar Farouk', 'Yousef Ahmed', 'Ali Hassan', 'Khaled Mostafa', 'Nourhan Sami', 'Tarek Ibrahim', 'Fatima Zahra', 'Ahmed Samy', 'Omar Ashraf', 'Mariam Adel', 'Noha Farouk', 'Ahmed Mostafa', 'Sara Ahmed', 'Heba Mohamed', 'Cairo, Egypt', 'Cairo, EG', 'LinkedIn', 'Sarah Ahmed', 'SA', Unsplash avatars) were replaced with dynamic data mapped from `apiApplications`. Filter dropdowns (positions, stages, owners, sources) now derive options dynamically from live applications, and filter defaults reset to `'ALL'`. Clean `<PageState kind="loading" | "error" | "empty">` states are integrated for both Kanban and List views.
- **StageTransitionPage Optimistic Concurrency & P0.1 Contract Alignment**: Previously, `StageTransitionPage` had a mock transition patch (`stage: 'Interview', reason: '...'`) that omitted `expectedStage` and `expectedVersion`, swallowed errors, and set a premature success message. In E4b, `handleConfirmTransition` sends `{ stage: targetStage, expectedStage: application.stage, expectedVersion: application.version ?? 1, reason?: string }` matching the P0.1 contract. It handles 409 Conflict with a warning banner and refetch, surfaces non-409 errors in a retryable danger Alert, and only navigates on genuine success. Hardcoded candidate literals ('Ali Hassan', 'Frontend Developer', 'APP-02481', 'Sarah Ahmed', 'SA', 'AH', static dates) were replaced with dynamic fields from `application` and `candidate`. The "To stage" box provides an interactive selector populated from `application.allowedTransitions` (or standard pipeline stages).

## 12. Enhancements Sweep E7 Findings (Per-Position Operational Parity Blueprint)
- **Direct Candidate Sourcing Modal (`AddApplicationModal.tsx`)**: Provides 1-click candidate addition directly into a requisition pipeline. Dual-mode support: existing candidate search (`GET /candidates`) with quick selection or new candidate creation (`POST /candidates`) with validation (firstName, lastName, email, phone, currentTitle). Enforces vacancy locking context badge when invoked from within a requisition, or an interactive requisition selector when called globally. Posts `POST /applications` with `{ candidateId, vacancyId, source, stage: 'Applied' }`, and optionally appends an initial recruiter note via `POST /applications/:id/notes`.
- **Position-Scoped Sub-flows for Interviews & Offers**: Both `InterviewsPage.tsx` and `OffersPage.tsx` now accept `?vacancyId=` search params. When scoped, `InterviewsPage` resolves vacancy applicant IDs via `GET /vacancies/:id` and filters both the interview list and the scheduling modal candidate dropdown to only that position's applicants. `OffersPage` filters the table rows by `vacancyId`. Both display prominent SGH-styled context banners with one-click return links (`[← Back to Requisition Overview]`) and clear filters (`[View All]`).
- **Odoo-Style Position Cards & Grid View**: `VacantListPage.tsx` now defaults to a 3-column responsive card grid (`viewMode === 'cards'`) with toggle switch to Table view. Cards feature requisition codes (`vacancyCode`), status pill badges, SLA tracking indicators (`slaPercent` & status dot), approved vs joined headcount metrics (`joinedHeadcount / approvedHeadcount filled`), recruiter avatar badges, and a prominent 1-click `[X Applications ↗]` button that jumps directly into the vacancy-locked pipeline.
- **Requisition Bidirectional Linkage & Inline Evaluation Tab**: `ApplicationDetailPage.tsx` now features a clickable requisition breadcrumb and a top-right `[Requisition Overview ↗]` button navigating directly to `/vacancies/${application.vacancyId}`. The premature eject behavior on clicking the `interviews` tab (`if (tab === 'interviews') navigate('/interviews')`) has been eliminated in favor of an inline evaluation tab rendering scorecard metrics, interview schedule history, interviewer assignments, and individual scorecard ratings.
