...
import type { CreatePipelineTemplateDto, UpdatePipelineTemplateDto, CreatePipelineStageDto, UpdatePipelineStageDto, ReorderStagesDto } from './pipeline-settings.dto';
...
export class PipelineSettingsService {
  ...
  async listTemplates(organizationId: string): Promise<PipelineTemplateItem[]> {
    const templates = await this.prisma.pipelineTemplate.findMany({
      ...
      include: {
        _count: {
          select: { stages: { where: { status: { not: 'Archived' } } } }
        }
      }
    });
  ...
  async createTemplate(organizationId: string, dto: CreatePipelineTemplateDto) {
    return this.prisma.$transaction(async (tx) => {
      if (dto.isDefault) {
        await tx.pipelineTemplate.updateMany({ where: { organizationId, isDefault: true }, data: { isDefault: false } });
      }
      return tx.pipelineTemplate.create({
        data: { organizationId, name: dto.name.trim(), isDefault: dto.isDefault ?? false, status: 'Active' },
      });
    });
  }

  async updateTemplate(organizationId: string, id: string, dto: UpdatePipelineTemplateDto) {
    const template = await this.prisma.pipelineTemplate.findFirst({ where: { id, organizationId, status: { not: 'Archived' } } });
    if (!template) throw new NotFoundException('Template not found');
    return this.prisma.$transaction(async (tx) => {
      if (dto.isDefault) {
        await tx.pipelineTemplate.updateMany({ where: { organizationId, id: { not: id }, isDefault: true }, data: { isDefault: false } });
      }
  ...
  async archiveTemplate(organizationId: string, id: string) {
    const template = await this.prisma.pipelineTemplate.findFirst({ where: { id, organizationId, status: { not: 'Archived' } } });
    if (!template) throw new NotFoundException('Template not found');
    if (template.isDefault) throw new ConflictException('The default template cannot be archived. Set another template as default first.');
    return this.prisma.pipelineTemplate.update({ where: { id }, data: { status: 'Archived', isDefault: false } });
  }

  async duplicateTemplate(organizationId: string, id: string) {
    const template = await this.prisma.pipelineTemplate.findFirst({
      where: { id, organizationId, status: { not: 'Archived' } },
      include: { stages: { where: { status: { not: 'Archived' } } } }
    });
  ...
  async addStage(organizationId: string, templateId: string, dto: CreatePipelineStageDto) {
    ...
    const template = await this.prisma.pipelineTemplate.findFirst({
      where: { id: templateId, organizationId, status: { not: 'Archived' } }
    });
    if (!template) throw new NotFoundException('Template not found');
  ...
  async updateStage(organizationId: string, templateId: string, stageId: string, dto: UpdatePipelineStageDto) {
    ...
    return this.prisma.pipelineStage.update({
      ...
      data: {
        ...(dto.name !== undefined && { name: dto.name.trim() }),
        ...(dto.stageType !== undefined && { stageType: dto.stageType.trim() }),
  ...
  async deleteStage(organizationId: string, templateId: string, stageId: string) {
    ...
    const stageCount = await this.prisma.pipelineStage.count({ where: { templateId, status: { not: 'Archived' } } });
    if (stageCount <= 1) throw new ConflictException('A pipeline must retain at least one active stage.');
    return this.prisma.pipelineStage.update({ where: { id: stageId, templateId }, data: { status: 'Archived' } });
  }
}
