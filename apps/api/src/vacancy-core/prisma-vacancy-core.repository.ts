...
export class PrismaVacancyCoreRepository implements VacancyCoreRepository {
  ...
  async createRequest(input: CreateVacancyRequestInput): Promise<VacancyRequest> {
    ...
    const request = await this.prisma.vacancyRequest.create({
      data: {
        ...
        targetFillDate: toDate(input.targetFillDate),
        recruitmentTiming: input.recruitmentTiming ?? null,
        plannedOpenDate: toDate(input.plannedOpenDate),
  ...
  private async saveRequestInTransaction(
    ...
    await transaction.vacancyRequest.update({
      ...
      data: {
        ...
        targetFillDate: toDate(request.targetFillDate),
        recruitmentTiming: request.recruitmentTiming,
        plannedOpenDate: toDate(request.plannedOpenDate),
  ...
  private toVacancyRequest(request: NonNullable<RequestWithApprovals>): VacancyRequest {
    return {
      ...
      budgetMin: request.budgetMin,
      budgetMax: request.budgetMax,
      budgetCurrency: request.budgetCurrency as VacancyRequest['budgetCurrency'],
      criticality: request.criticality,
      targetStartDate: toDateOnlyString(request.targetStartDate),
      targetFillDate: toDateOnlyString(request.targetFillDate),
      recruitmentTiming: request.recruitmentTiming as VacancyRequest['recruitmentTiming'],
      plannedOpenDate: toDateOnlyString(request.plannedOpenDate),
      justification: request.justification,
      jobSummary: request.jobSummary,
      description: request.description,
      responsibilities: request.responsibilities,
