...
describe('ImportJobDescriptionModal', () => {
  ...
  it('populates fields when a file is parsed and allows adding/removing skill chips', async () => {
    ...
    expect(screen.getByDisplayValue('SGH, UAE')).toBeInTheDocument();
  ...
  it('commits Master Data sync and updates target vacancy on submit', async () => {
    const user = userEvent.setup();
    ...
    await waitFor(() => {
      ...
      expect(mockPatchApi).toHaveBeenCalledWith('/vacancies/vac-101', expect.objectContaining({
        title: 'HRIS Performance Specialist',
        department: 'Human Resources',
        location: 'SGH, UAE',
        minExperienceYears: 3,
